    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if (
            ...
            if(args.freeze_param):                
                if(args.freeze_param[0] == 'whisper_pe'):
                    for k, p in model.named_parameters():
                        if k.find('cs') ==-1 :
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'adapter'):
                    for k, p in model.named_parameters():
                        if k.find('adapter') ==-1 :
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                        if k.find('estimated_c_val') !=-1:
                            logging.info(f"Setting {k}.requires_grad = True")
                            p.requires_grad = True
                elif(args.freeze_param[0] == 'whisper_pe_adapter'):
                    for k, p in model.named_parameters():
                        if k.find('adapter') ==-1 and k.find('cs') ==-1 :
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'freeze_decoder_pe'):
                    for k, p in model.named_parameters():
                        if (k.find('decoder') !=-1 and  k.find('cs') ==-1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'freeze_decoder_adapter'):
                    for k, p in model.named_parameters():
                        if (k.find('decoder') !=-1 and  k.find('adapter') ==-1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'all_param'):
                    for k, p in model.named_parameters():
                        logging.info(f"Setting {k}.requires_grad = False")
                        p.requires_grad = False
                elif(args.freeze_param[0] == 'sidenetwork'):
                    for k, p in model.named_parameters():
                        if(k.find('sidenetwork')==-1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                        # if(k.find('token_embedding')==1):
                        #     logging.info(f"Setting {k}.requires_grad = True")
                        #     p.requires_grad = True
                elif(args.freeze_param[0] == 'decoder_sidenetwork'):
                    for k, p in model.named_parameters():
                        if(k.find('encoders_sidenetwork')==-1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'adapter_encoder'):
                    for k, p in model.named_parameters():
                        if (k.find('encoder') !=-1 and  k.find('adapter') ==-1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                        if (k.find('decoder') != -1):
                            logging.info(f"Setting {k}.requires_grad = False")
