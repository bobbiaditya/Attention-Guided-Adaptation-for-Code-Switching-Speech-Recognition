...
class Speech2Text:
    ...
    def __init__(
        ...
        if bpemodel not in ["whisper_en", "whisper_multilingual"]:
            ...
            # modify here
            new_prefix_tokens = [50258, 50260, 50259, 50359, 50363] # sot zh en asr notimestamps
            # new_prefix_tokens = [50258, 50260, 50359, 50363] # sot zh asr notimestamps
            beam_search.set_hyp_primer(
                list(new_prefix_tokens)
            )
            # beam_search.set_hyp_primer(
            #     list(converter.tokenizer.prefix_tokens)
            # )
    ...
    def __call__(
        ...
        if self.asr_model.encoder.sidenetwork:
            enc, _, enc_sidenetwork = self.asr_model.encode(**batch)
        else:
        ...
        if self.enh_s2t_task or self.multi_asr:
            ...
            # c. Passed the encoder result and the beam search
            if self.asr_model.encoder.sidenetwork:
                results = self._decode_single_sample(enc[0],enc_sidenetwork[0])
            else:
    ...
    def _decode_single_sample(self, enc: torch.Tensor,enc_sidenetwork:torch.Tensor=None):
        if self.beam_search_transducer:
            ...
            if(self.asr_model.encoder.sidenetwork):
                nbest_hyps = self.beam_search(
                    x=enc, maxlenratio=self.maxlenratio, minlenratio=self.minlenratio,x_enc=enc_sidenetwork
                )
            else:
                nbest_hyps = self.beam_search(
