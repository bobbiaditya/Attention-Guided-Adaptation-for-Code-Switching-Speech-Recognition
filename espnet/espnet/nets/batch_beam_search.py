...
class BatchBeamSearch(BeamSearch):
    ...
    def score_full(
        self, hyp: BatchHypothesis, x: torch.Tensor , x_enc:torch.Tensor=None
        ...
        for k, d in self.full_scorers.items():
            if(x_enc is not None):
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], x, x_enc)
            else:
    ...
    def search(self, running_hyps: BatchHypothesis, x: torch.Tensor,x_enc:torch.Tensor=None) -> BatchHypothesis:
        ...
        if x_enc is not None:
            scores, states = self.score_full(running_hyps, x.expand(n_batch, *x.shape), x_enc.expand(n_batch, *x_enc.shape))
        else:
